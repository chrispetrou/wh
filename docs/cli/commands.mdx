...
`wh init zsh` (or `bash`, `fish`) prints a small `wh()` function that forwards every command and turns `wh switch` into a `cd`. Nothing is written; you `eval` it from your rc file. See [install](/docs/install#shell-wrapper).
