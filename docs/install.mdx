...
The `cd` only happens in an interactive shell. A script, or a coding agent's shell that loaded your rc file, gets the path on stdout like the bare binary, so `cd "$(wh switch <name>)"` works there too.
